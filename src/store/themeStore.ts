...
export const useThemeStore = create<ThemeState>((set, get) => ({
    ...
    setActiveThemeId: async (id: string) => {
        ...
        if (target) {
            ...
            await invoke('set_active_theme', { themeId: id }).catch((err) =>
    ...
    fetchThemes: async () => {
        ...
        try {
            const [themes, activeThemeId] = await Promise.all([
                ...
                invoke<string>('get_active_theme_id').catch(() => 'veyak-dark'),
            ]);
    ...
    importTheme: async (filePath: string) => {
        try {
            const imported = await invoke<Theme>('import_theme_file', { path: filePath });
            set((state) => ({
    ...
    exportTheme: async (themeId: string, destinationDir: string) => {
        await invoke('export_theme_file', { themeId, destinationDir }).catch((err) =>
    ...
    deleteCustomTheme: async (themeId: string) => {
        try {
            ...
            set((state) => {
                ...
                const nextActive =
                    state.activeThemeId === themeId ? remaining[0] || null : state.activeTheme;
